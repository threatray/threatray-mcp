...
def register(mcp: FastMCP) -> None:
    ...
    async def threatray_get_file_metadata(ctx: Context, params: FileMetadataInput) -> str:
        ...
        result = await client.files.get_metadata(FileHashAny(params.file_hash))
    ...
    async def threatray_get_strings(ctx: Context, params: StringsInput) -> str:
        ...
        client = get_client(ctx)
        if params.response_format == ResponseFormat.JSON:
            result = await client.files.get_strings(FileHashAny(params.file_hash))
            return format_json(result)
        # Fast path: probe with limit=201 so we can tell ≤200 (render all,
        # no spill) from >200 (one extra unbounded fetch, then spill).
        probe = await client.files.get_strings(FileHashAny(params.file_hash), limit=_MARKDOWN_STRINGS_PROBE)
        ...
        full = await client.files.get_strings(FileHashAny(params.file_hash))
    ...
    async def threatray_download_file(ctx: Context, params: FileDownloadInput) -> str:
        ...
        """
        client = get_client(ctx)
        data = await client.files.download(FileHashAny(params.file_hash), zipped=True)
